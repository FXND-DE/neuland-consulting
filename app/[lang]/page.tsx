...
const baseUrl = 'https://neuland.consulting'
...
const buildStructuredData = (lang: Locale, t: Translation) => {
  ...
  const description = stripMarkup(t.subtitle)
  ...
  return {
    ...
    founder: {
      ...
      email: 'mailto:felix@neuland.consulting',
    },
    address: {
      '@type': 'PostalAddress',
    ...
    contactPoint: [
      {
        '@type': 'ContactPoint',
        contactType: lang === 'de' ? 'Beratung' : 'Consulting',
        email: 'mailto:felix@neuland.consulting',
        availableLanguage: ['de', 'en'],
      },
    ],
    hasOfferCatalog: {
      '@type': 'OfferCatalog',
      name: lang === 'de' ? 'Beratungsleistungen' : 'Consulting services',
...
export async function generateMetadata({
  ...
  const currentLang: Locale = lang === 'de' ? 'de' : 'en'
  const t = translations[currentLang]

  const description = stripMarkup(t.subtitle)
  const localeTag = currentLang === 'de' ? 'de-DE' : 'en-US'
  ...
  return {
    ...
    openGraph: {
      ...
      title: t.title,
      description,
      siteName: 'neuland.consulting',
      locale: localeTag,
      alternateLocale: currentLang === 'de' ? ['en_US'] : ['de_DE'],
      images: [
        {
          url: `${baseUrl}/NC_Back.png`,
          alt: stripMarkup(t.title),
        },
        {
...
export default function Home({ params }: { params: { lang?: string } }) {
  ...
  const structuredDataObject = buildStructuredData(currentLang, t)
      ...
      <main className="min-h-screen bg-white text-gray-900 font-sans">
        ...
        <section className="px-6 py-18 text-center bg-white">
          ...
          <p className="text-lg text-gray-700 mb-8 leading-relaxed">
            {currentLang === 'de'
              ? parseHighlight(
                  '<hl>Strategische Beratung</hl> an der Schnittstelle von <hl>Technologie</hl>, <hl>Kommunikation</hl> und <hl>Business</hl>'
                )
              : parseHighlight(
                  '<hl>Strategic consulting</hl> at the intersection of <hl>technology</hl>, <hl>communication</hl>, and <hl>business</hl>'
                )}
          </p>
